    ...
    pub fn run_ready_task(&self) {
        ...
        if let Some(task) = task {
            ...
            *self.cores[cur_cpu_id].lock() = Some(task.clone());
            info!("task : {:?}", self.cores[cur_cpu_id].lock());
            ...
            match future.as_mut().poll(&mut context) {
                ...
                Poll::Pending => {
                    info!("Task {:?} pending, returning to queue", task_id);
...
pub fn get_cur_usr_task() -> Arc<UserTask>
{
    let executor = GLOBLE_EXECUTOR.lock();
    let task_option_guard = executor.cores[get_cur_cpu_id()].lock();
    let task_arc = task_option_guard.as_ref().expect("No current task").clone();
    task_arc.downcast_arc::<UserTask>().expect("REASON")
}
