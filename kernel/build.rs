...
fn gen_linker_script() -> Result<()> {
    ...
    let ld_content = std::fs::read_to_string("linker.ld")?;
    let ld_content = ld_content.replace("%ARCH%", output_arch);
    let ld_content = ld_content.replace("%KERNEL_BASE%", kernel_base);

    // Get the output directory from Cargo environment variables
    let out_dir = env::var("OUT_DIR").expect("Failed to get OUT_DIR");
    let script_path = format!("{}/{}", out_dir, fname);
    
    // Write the file to the output directory
    std::fs::write(&script_path, ld_content)?;
